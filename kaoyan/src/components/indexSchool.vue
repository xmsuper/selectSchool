                ...
                <el-col :span="14" @click="gotoDetail(n.school_id)">
                    <div>
                        ...
                        <el-breadcrumb separator="|">
                            ...
                            <el-breadcrumb-item v-if="n.is_211==1">211</el-breadcrumb-item>
                            <el-breadcrumb-item v-if="n.is_985==1">985</el-breadcrumb-item>
                            <el-breadcrumb-item v-if="n.syl==1">双一流</el-breadcrumb-item>
                            <el-breadcrumb-item v-if="n.is_zihuaxian==1">自划线</el-breadcrumb-item>
                        </el-breadcrumb>
                    </div>
                </el-col>
                <el-col :span="8">
                    ...
                    <div class="schoolListBtn">
                        ...
                        <el-button type="warning" @click="gotoScore(n.school_id)">分数线</el-button>
...
// toRefs把传递过来的数据的每一项都变成响应式的
// const { chooseCondition } = toRefs(props)
// 接收父组件传递过来的数据 ---结束  
...
watch(props.chooseCondition, () => {
    // console.log(props.chooseCondition)
    const notNull=Object.fromEntries(
        Object.entries(props.chooseCondition).filter(([k,v])=>v!==null&&v!=='')
    )
    // console.log(notNull)
    requestFn({
        url: '/detail_school_list',
        method: 'post',
        data: {
            data:notNull
        }
        ...
        schoolInfo.arr = data.data
        // console.log(schoolInfo.arr)
        maxPage.value = parseInt(data.data.length)
        schoolInfoSlice.arr = schoolInfo.arr.slice((curretPage.value - 1) * pageSize.value, curretPage.value * pageSize.value)
    }).catch(error=>{
        console.log(error)
    })
