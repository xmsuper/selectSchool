...
from kaoyan_houduan.settings import BASE_DIR
...
from showSchool.models import SchoolImg, SchoolInfo, SchoolType, SchoolScore, \
    ...
    , SchoolDetail, SylBuild, MajorDetail, CountryLine, Image,article,article_comment,answer
    ...
    def post(self, request):
        ...
        if request.FILES.get('file'):
            ...
            if Userinfo.objects.filter(username=name).exists():
                ...
                currentUser.update(avator=uploaded_file_url,username=modifyName,currentSchool=currentSchool,gender=gender,grade=year,join_school=joinSchool,join_major=joinMajor,join_type=learnType,learn_major=currentMajor)
            ...
            if Userinfo.objects.filter(username=name).exists():
                currentUser=Userinfo.objects.filter(username=name)
                currentUser.update(username=modifyName,gender=gender,grade=year,currentSchool=currentSchool,join_school=joinSchool,join_major=joinMajor,join_type=learnType,learn_major=currentMajor)
...
class article_list(APIView):
    def get(self,request):
        data = []
        query = 'select article.*,userinfo.avator from article left join userinfo on article.username=userinfo.username order by article.cur_time desc'
        with connection.cursor() as cursor:
            cursor.execute(query)
            res = cursor.fetchall()
            for i in res:
                sql='select count(*) from article_comment where article_id='+str(i[2])
                cursor.execute(sql)
                ...
                data.append({
                    ...
                    'img_url': img_list,
                    'avator':i[6],
                    'comment_count':article_count[0]
                })
        return Response(data)
class getSingleArticle(APIView):
    def post(self,request):
        data = []
        article_id=request.data['params']['article_id']
        query = 'select article.*,userinfo.avator from article left join userinfo on article.username=userinfo.username where article.number='+str(article_id)
        with connection.cursor() as cursor:
            cursor.execute(query)
            res = cursor.fetchone()
            sql = 'select count(*) from article_comment where article_id=' + str(res[2])
            cursor.execute(sql)
            article_count = cursor.fetchone()
            img_list = ((res[0].decode('utf-8')).split(','))[:-1]
            data.append({
                'username': res[5],
                'content': res[1],
                'id': res[2],
...
class getUserArticle(APIView):
    def post(self,request):
        ...
        username=request.data['params']['username']
        print(username)
        query = 'select article.*,userinfo.avator from article left join userinfo on article.username=userinfo.username where article.username="'+username+'"'
        with connection.cursor() as cursor:
            cursor.execute(query)
            res = cursor.fetchall()
            for i in res:
...
class getAllComment(APIView):
    def get(self,request):
        ...
        return Response(data)
class getSingleComment(APIView):
    def post(self, request):
        article_id=request.data['params']['article_id']
        print(article_id)
        data = []
        query = 'select a.article_id,a.username,a.comment,b.avator from article_comment a left join userinfo b on a.username=b.username where a.article_id='+str(article_id)
        with connection.cursor() as cursor:
            cursor.execute(query)
            res = cursor.fetchall()
            for i in res:
...
from utils.final_test import SchoolRecommend
class recommend(APIView):
    def post(self,request):
        ...
        sql1='select school_rank,score,major_code,recruit_num,remark from school_remark where school_name="'+data['scoreSchool']+'" and major_name="'+data['major_class2']+'"'
        # 获取应该输入的推荐院校的信息
        # print(sql1)
        with connection.cursor() as cursor:
            cursor.execute(sql1)
            ...
            if first==None:
                return Response({'msg':'改院校没有改专业'})
            else:
                for i in first:
                    if(type(i)==str):
                        i=int(i[:3])
                    input_school.append(i)
        # print(input_school)
        # 调用推荐算法
        recommend_list=SchoolRecommend(input_school)
        new_recommend = []
        for i in recommend_list:
            # query = 'select distinct a.*,b.province_name,b.*,c.school_img from school_score a left join ' \
            #         'school_info b on a.school_name=b.school_name left join school_img c on a.school_name=c.school_name where province_name="' + \
            #         data['region'] + '" and learnType="' + data['class'] + '" and major_name="' + data[
            #             'major_class2'] + '"and b.school_id='+str(int(i))
            query='select a.*,left(a.total,3) as major_total,b.*,si.* from school_score a ' \
                  'left join school_info b on a.school_name = b.school_name left join school_img si on ' \
                  'b.school_name = si.school_name where school_id='+str(i)+' and major_name="'+\
                  data['major_class2']+'" and province_name="' + data['region'] + '" and learnType="' + data['class']+\
                  '" and left(a.total,3)<'+str(score+20)
            with connection.cursor() as cursor:
                cursor.execute(query)
                res = cursor.fetchone()
                if res:
                    recommend_school=list(res)
                    recommend_school[11]=int((recommend_school[11])[:3])
                    # recommend_school[11]=int(recommend_school[11])
                    # print(recommend_school)
                    new_recommend.append(tuple(recommend_school))
        print(new_recommend)
        sorted_students = sorted(new_recommend, key=lambda x: x[11])
        print(sorted_students)
        return Response(sorted_students)
...
class DeleteArticle(APIView):
    def post(self,request):
        username=request.data['params']['username']
        article_id=request.data['params']['article_id']
        deleteSuccess=article.objects.filter(username=username,number=article_id).delete()
        if not deleteSuccess:
            return Response({'code':500,'msg':'删除失败，请重试'})
        else:
            return Response({'code':200,'msg':'删除成功！'})

class showCompare(APIView):
    def post(self,request):
        data=request.data['params']
        compareSchool=data['compareSchool']
        major_name=data['major_name']
        print(compareSchool,major_name)
        school_data=[]
        for i in compareSchool:
            print(i)
            school_name_query=SchoolInfo.objects.filter(school_id=i).values('school_name')
            school_name=(school_name_query[0]['school_name'] )
            print(school_name)
            # query='select distinct a.*,b.total,c.recruit_number,c.recruit_name,c.special_name from school_info a left join school_score b on a.school_name=b.school_name left join major_detail c on a.school_id=c.school_id where c.special_name="'+major_name+'" and a.school_id='+str(i)
            query='select distinct first.*,second.recruit_number,second.recruit_type,second.school_id,third.*,firth.school_img from (select a.school_name,a.major_name,a.total,a.major_code from school_score a where school_name="'+school_name+'" and major_name="'+major_name+'") as first left join major_detail as second on first.major_name=second.level2 left join school_info as third on second.school_id=third.school_id left join school_img as firth on third.school_name=firth.school_name where second.school_id='+str(i)+';'
            print(query)
            with connection.cursor() as cursor:
                cursor.execute(query)
                res = cursor.fetchall()
                ...
                school_data.append({
                    'school_name':(res[0])[0],
                    'school_major':(res[0])[1],
                    'total':(res[0])[2],
                    'major_code':(res[0])[3],
                    'recruit_number':(res[0])[4],
                    'learn_type':(res[0])[5],
                    'school_id':(res[0])[6],
                    'is_211':(res[0])[7],
                    'syl':(res[0])[11],
                    'is_985':(res[0])[14],
                    'is_zihuaxian':(res[0])[15],
                    'rank':(res[0])[16],
                    'province_name':(res[0])[17],
                    'school_img':(res[0])[20]
                })
        return Response(school_data)

class getMajorSchool(APIView):
    def post(self,request):
...
class schoolScore_ser(serializers.ModelSerializer):
    class Meta:
        model=SchoolScore
        fields='__all__'
class kaisheyuanxiao(APIView):
    def post(self,request):
        data=request.data['params']
        print(data)
        query='select distinct a.*,b.*,c.* from school_score a left join school_info b on a.school_name=b.school_name left join school_img c on b.school_name=c.school_name where a.major_name="'+data['major_name']+'" and a.learnType="'+data['major_class']+'" and a.major_code="'+data['major_code']+'"'
        # print(query)
        school_data=[]
        with connection.cursor() as cursor:
            cursor.execute(query)
...
class wordCloud(APIView):
    def get(self,request):
        data=''
        content=article.objects.all().values('content')
        for i in content:
            data+=i['content']
        print(data)
        ...
        # 将词云图像保存到相对路径
        relative_path = 'media/wordcloud/wordcloud.png'  # 相对路径
        abs_path = os.path.join(BASE_DIR, relative_path)  # 绝对路径：BASE_DIR为Django项目根目录
        wc.to_file(abs_path)

        return Response({'wordcloud':relative_path})
class TF(APIView):
    def get(self,request):
        data=''
        content=article.objects.all().values('content')
        ...
        for word in words:
            if word not in chinese_stopwords and len(word) > 1:  # 过滤停用词和单个字的词
                word_count[word] += 1
        ...
        final_word=[]
        # 排序并输出常出现的词
        top_words = sorted(tf.items(), key=lambda x: x[1], reverse=True)[:6]
        for word, frequency in top_words:
            print(word, frequency)
            final_word.append(word)
        return Response({'final_word':final_word})
class zuoti_ser(serializers.ModelSerializer):
    class Meta:
        model=answer
        fields='__all__'
class zuoti(APIView):
    def get(self,request):
        ...
        serializer=zuoti_ser(instance=query,many=True)
        return Response(serializer.data)
...
class checkAnswer(APIView):
    def post(self,request):
        data=request.data['params']
        print(data)
        curVal=data['chooseVal']
        id=data['id']
        rightAnswer=answer.objects.filter(id=id).first()
        rightAnswerVal=((rightAnswer.answer)[4:])
        if curVal==rightAnswerVal:
            print(curVal,rightAnswer.answer)
            return Response({'code':200,'msg':'回答正确'})
        else:
            print(curVal,rightAnswer.answer)
            return Response({'code':404,'msg':'回答错误'})
